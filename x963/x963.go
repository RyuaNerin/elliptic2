package x963

import "github.com/RyuaNerin/elliptic2"

// Ansit163k1 returns a Curve which implements ANSI X9.63 ansit163k1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit163k1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit163k1
}

// Ansit163r1 returns a Curve which implements ANSI X9.63 ansit163r1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit163r1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit163r1
}

// Ansit163r2 returns a Curve which implements ANSI X9.63 ansit163r2
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit163r2() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit163r2
}

// Ansit193r1 returns a Curve which implements ANSI X9.63 ansit193r1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit193r1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit193r1
}

// Ansit193r2 returns a Curve which implements ANSI X9.63 ansit193r2
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit193r2() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit193r2
}

// Ansit233k1 returns a Curve which implements ANSI X9.63 ansit233k1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit233k1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit233k1
}

// Ansit233r1 returns a Curve which implements ANSI X9.63 ansit233r1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit233r1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit233r1
}

// Ansit239k1 returns a Curve which implements ANSI X9.63 ansit239k1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit239k1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit239k1
}

// Ansit283k1 returns a Curve which implements ANSI X9.63 ansit283k1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit283k1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit283k1
}

// Ansit283r1 returns a Curve which implements ANSI X9.63 ansit283r1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit283r1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit283r1
}

// Ansit409k1 returns a Curve which implements ANSI X9.63 ansit409k1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit409k1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit409k1
}

// Ansit409r1 returns a Curve which implements ANSI X9.63 ansit409r1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit409r1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit409r1
}

// Ansit571k1 returns a Curve which implements ANSI X9.63 ansit571k1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit571k1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit571k1
}

// Ansit571r1 returns a Curve which implements ANSI X9.63 ansit571r1
//
// Multiple invocations of this function will return the same value, so it can
// be used for equality checks and switch statements.
//
// The cryptographic operations are implemented using constant-time algorithms.
func Ansit571r1() elliptic2.Curve {
	initonce.Do(initAll)
	return ansit571r1
}
